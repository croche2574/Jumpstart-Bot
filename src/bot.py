...
class Pack():
    def __init__(self, name, url, emoji, description) -> None:
        self.pack_name = name
        self.url = url
...
class PackSelect(discord.ui.Select):
    def __init__(self, packs, placeholder, view, disabled=False, next_sel=None, submit_butt=None):
        self.packs = packs
        self.next_select = next_sel
        self.submit = submit_butt
        options = [
...
class PackSelectView(discord.ui.View):
    def __init__(self, player_packs):
        ...
        submit = SubmitButton()
        cancel = CancelButton()
        pack_select_2 = PackSelect(self.player_packs[3:], "Select Pack 2", self, True, submit_butt=submit)
        pack_select_1 = PackSelect(self.player_packs[:3], "Select Pack 1", self, False, pack_select_2)
...
class JoinView(discord.ui.View):
    def __init__(self, time, packs, max_players, author, support_pack=None):
        super().__init__(timeout=time*60)
        self.time = time
        self.player_list = []
    ...
    async def join_callback(self, button, interaction):
        ...
        view = PackSelectView(player.packs)
        eph_msg = await interaction.followup.send("### Choose two packs and then click Submit!", view=view, ephemeral=True)
        await view.wait()
        
        if view.cancelled:
            self.player_list.remove(player)
            self.pack_list.extend(player.packs)
        else:
            player.packs.sort(key=lambda x: x.selected, reverse=True)
            print(interaction.user.name + ": \n" + ''.join([str(i) for i in player.packs]))
            self.pack_list.extend(player.packs[2:])
            player.packs = player.packs[:4]
        ...
        await eph_msg.edit(content="", embed=embed, view=None)
        # print(len(self.pack_list))
...
class JumpStartManagerBot(discord.Bot):
    ...
    async def start_draft(self, ctx, draft_timer, num_players):
        ...
        if self.pack_list:
            # If not using support pack, comment this line out and edit the response.
            support_pack = self.pack_list.pop()
            await ctx.respond(embed=embed, view=JoinView(draft_timer, self.pack_list[:], num_players, self.author, support_pack=support_pack))
        else:
